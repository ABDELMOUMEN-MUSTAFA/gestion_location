$(document).ready(function (){
	...
	function validationPassword(mdp1, mdp2) {
		if(mdp1 === '' || mdp2 === ''){
			$('.mdp2').prepend('<i class="fas fa-exclamation-circle trailing text-danger"></i>');
			showErrorMessage('mdp1', `les deux champs du mot de passe sont obligatoire !`);
			return false;
		}else{
			if(mdp1.length < 10){
				$('.mdp2').prepend('<i class="fas fa-exclamation-circle trailing text-danger"></i>');
				showErrorMessage('mdp1', `le nombre minimum est 10 caracteres !`);
				return false;
			}else{
				if(mdp1 !== mdp2){
					$('.mdp2').prepend('<i class="fas fa-exclamation-circle trailing text-danger"></i>');
					showErrorMessage('mdp1', 'les deux champs mot de passe doit etre identique !');
					return false;
				}

				removeErrorMessage('mdp1');
				removeErrorMessage('mdp2');
				return true;
			}
		}
	}
	...
	$form.submit(function(e) {
		...
		isAllValid[9] = validationPassword($mdp1.val(), $mdp2.val());
